...
class TaxHistoryControllerSpec
  ...
  trait Setup {
    ...
    val underTest                                = new TaxHistoryController(mock[ScenarioLoader], mock[TaxHistoryService], stubControllerComponents())
  ...
  "find" should {
    "return 200 (Ok) with the response when called with a nino and taxYear that are found" in new Setup {
      ...
      val result: Future[Result] =
        Future(underTest.find(nino, taxYear.startYr.toInt)(createTaxHistoryRequest)).futureValue
  ...
  "create" should {
    "return 406 Not Acceptable when called with version 1" in new Setup {
      val result: Future[Result] =
        Future(underTest.create(nino, taxYear)(createSummaryRequestV1("EVERYTHING"))).futureValue
    ...
    "return a created response and store the tax history" in new Setup {
      ...
      given(underTest.scenarioLoader.loadScenarioRaw(mEq("tax-history"), mEq("EVERYTHING")))
        .willReturn(Future.successful(taxHistoryResponse))
      given(underTest.service.create(mEq(nino), mEq(taxYear), mEq(taxHistoryResponse)))
        .willReturn(Future.successful(taxHistory))

      val result: Future[Result] =
        Future(underTest.create(nino, taxYear)(createSummaryRequest("EVERYTHING"))).futureValue
    ...
    "return an Internal Server Error when the repository fails" in new Setup {
      ...
      given(underTest.scenarioLoader.loadScenarioRaw(mEq("tax-history"), mEq("EVERYTHING")))
        .willReturn(Future.successful(taxHistoryResponse))
      given(underTest.service.create(any(), any(), anyString))
    ...
    "return a bad request when the scenario is invalid" in new Setup {
      ...
      given(underTest.scenarioLoader.loadScenarioRaw(mEq("tax-history"), mEq("INVALID")))
